...
export function Settings({ onBack }: SettingsProps) {
  ...
  const [selectedCurrency, setSelectedCurrency] = useState<Currency>((user?.currency as Currency) || "USD");
  ...
  const handleChangeCurrency = async (e: React.FormEvent) => {
    ...
    try {
    const response = await api.auth.changeCurrency(selectedCurrency);
    updateCurrency(response.currency);
    setCurrencySuccess(true);
    setTimeout(() => setCurrencySuccess(false), 3000);
  } catch {
    setCurrencyError("Failed to change currency.");
  } finally {
